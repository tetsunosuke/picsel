...
def new
  @order = Order.new
  @trading_status = TradingStatus.new
end
def confirm
  if params['payjp-token'] == ''
    flash[:notice] = 'payjp token is empty'
    @order = current_user.orders.new(order_params)
    @trading_status = @order.trading_statuses.new(trading_create_status_params)
    return render :new
...
def create
    ...
    @trading_status = @order.trading_statuses.new(trading_status_create_params)
    binding.pry
    @trading_status.trading_status = 0
    @order.status = 0
...
def sold
  ...
  private
  def order_params
      params.require(:order).permit(:cart_item_id,:postal_code,:address,:name,:payment_method,:total_payment)
    end
    def order_create_params
      params.require(:order).permit(:postal_code,:address,:name,:payment_method,:total_payment,:buyer_id)
    end
    # def trading_status_create_params
    #   params.require(:trading_status).permit(:trading_status)
    # end
end
